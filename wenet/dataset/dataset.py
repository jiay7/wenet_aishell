...
from wenet.dataset.wav_distortion import distort_wav_conf,AugmentWAV
...
def _extract_feature(batch, speed_perturb, wav_distortion_conf,
                     feature_extraction_conf,augment_wav):
    ...
    augment = wav_distortion_conf['augment']
    if speed_perturb:
        speeds = [1.0, 1.1, 0.9]
        weights = [1, 1, 1]
    ...
    for i, x in enumerate(batch):
        try:
            ...
            if augment:
                ...
                if augtype == 1:
                    waveform = augment_wav.reverberate(waveform)
                elif augtype == 2:
                    waveform = augment_wav.additive_noise('music',waveform)
                elif augtype == 3:
                    waveform = augment_wav.additive_noise('speech',waveform)
                elif augtype == 4:
                    waveform = augment_wav.additive_noise('noise',waveform)
                waveform = torch.from_numpy(waveform)
            mat = kaldi.fbank(
                waveform,
...
class CollateFunc(object):
    ...
    def __init__(self,
        ...
        if self.wav_distortion_conf["augment"]:
            musan_path = self.wav_distortion_conf["musan_path"]
            rir_path = self.wav_distortion_conf["rir_path"]
            #max_frames = int(self.wav_distortion_conf["max_frames"])
    ...
    def __call__(self, batch):
        ...
        # pad_sequence will FAIL in case xs is empty 把不等长的batch padding 0作为等长。
        if len(xs) > 0:
            xs_pad = pad_sequence([torch.from_numpy(x).float() for x in xs],
                                  True, 0) #batch first == True
        else:
            xs_pad = torch.Tensor(xs)
        
        if train_flag: #train_flag代表是不是train的模式
            ...
            if len(ys) > 0:
                ys_pad = pad_sequence([torch.from_numpy(y).int() for y in ys],
                                      True, IGNORE_ID) #同样把不等长的batch标注 padding -1作为等长
            else:
                ys_pad = torch.Tensor(ys)
        else:
            ys_pad = None
            ys_lengths = None
        return keys, xs_pad, ys_pad, xs_lengths, ys_lengths # xs_lengths, ys_lengths保留batch内每个序列的长度用于ctc的loss
        ...
        with codecs.open(data_file, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                if raw_wav:
                    ...
                    #print((key, wav_path, duration, tokenid))
...
if __name__ == '__main__':
    ...
    for i, batch in enumerate(data_loader):
        #print(batch)
        keys, xs_pad, ys_pad, xs_lengths, ys_lengths = batch
        print(keys)
        print(xs_pad.shape)
        print(ys_pad.shape)
        print(xs_lengths)
