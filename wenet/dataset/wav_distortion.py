...
class AugmentWAV(object):
    ...
    def additive_noise(self, noisecat, audio):
        ...
        for noise in noiselist:
            ...
            if noise_length <= audio_length:
                ...
                noiseaudio  = numpy.pad(noiseaudio, ((0,0),(0,shortage)), 'wrap')
                noiseaudio = noiseaudio[:,:audio_length]
            else:
                startframe = numpy.int64(random.random()*(noise_length-audio_length))
                noiseaudio = noiseaudio[:,int(startframe):int(startframe)+audio_length]
            noise_snr   = random.uniform(self.noisesnr[noisecat][0],self.noisesnr[noisecat][1])
            noise_db = 10 * numpy.log10(numpy.mean(noiseaudio[0] ** 2)+1e-4) 
            noises.append(numpy.sqrt(10 ** ((clean_db - noise_db - noise_snr) / 10)) * noiseaudio)
        noise_sum = numpy.sum(numpy.concatenate(noises,axis=0),axis=0,keepdims=True)
    ...
    def reverberate(self, audio):
        ...
        audio_length = audio.shape[1]
        rir,fs     = torchaudio.load_wav(rir_file)
        rir = rir.detach().numpy()
        rir_length = rir.shape[1]
        if rir_length <= audio_length:
            shortage    = rir_length - rir_length + 1
            rir       = numpy.pad(rir, ((0,0),(0, shortage)), 'wrap')
        else:
            startframe = numpy.int64(random.random()*(rir_length-audio_length))
