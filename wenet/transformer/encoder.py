...
class BaseEncoder(torch.nn.Module):
    ...
    def forward_chunk(
        ...
        """
        assert xs.size(0) == 1 #assert Batch == 1
        ...
        if subsampling_cache is not None:
            ...
            cache_size = 0
        pos_emb = self.embed.position_encoding(offset - cache_size, xs.size(1)) #得到一个chunk的位置编码
    ...
    def forward_chunk_by_chunk(
        ...
        context = self.embed.right_context + 1  # Add current frame # 6+1=7
        stride = subsampling * decoding_chunk_size # 3*16=48
        decoding_window = (decoding_chunk_size - 1) * subsampling + context # 15*3+7=52
        num_frames = xs.size(1) #总帧数
        subsampling_cache: Optional[torch.Tensor] = None
        elayers_output_cache: Optional[List[torch.Tensor]] = None
        conformer_cnn_cache: Optional[List[torch.Tensor]] = None
        outputs = []
        offset = 0
        required_cache_size = decoding_chunk_size * num_decoding_left_chunks # 16* -1 = -16
        # Feed forward overlap input step by step
        for cur in range(0, num_frames - context + 1, stride):
            end = min(cur + decoding_window, num_frames)
            chunk_xs = xs[:, cur:end, :] #B,t,dim
